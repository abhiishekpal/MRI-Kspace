...
def refine_net(input_):
    ...
    decoded =  tf.layers.conv2d(inputs = conv9_5, filters = 3, kernel_size = 3, strides = 1, padding = "same", activation = tf.nn.tanh, name = "decoded_ref")
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epochs):
            ...
            for i in range(0,18):
                ...
                epoch_loss += c
                cv2.imwrite("./results2/img_{}_{}_gt.png".format(epoch,i),img_gt[8,:,:,0]*255.0)
                cv2.imwrite("./results2/img_{}_{}_gen.png".format(epoch,i),img_gen[8,:,:,0]*255.0)
                cv2.imwrite("./results2/img_{}_{}_un.png".format(epoch,i),img_un[8,:,:,0]*255.0)
